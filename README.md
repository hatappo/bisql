...
- Rename the SQL files to `<last-ns>.<fn-name>.sql`. This ensures that the clj and sql files generated by gen-ns are in the same folder, allowing for collocation.
  - CRUD SQL file naming: `sql/<dialect>/<schema>/<table>/crud.sql`  `/*:name crud.get-by-id */`
  -   ------------------> `sql/<dialect>/<schema>/<table>/crud.clj`  `(defn get-by-id ...)`
  - --------------------- `sql/<dialect>/<schema>/<table>/hoge.list-order-by-created-at.sql`
  -   ------------------> `sql/<dialect>/<schema>/<table>/hoge.clj`  `(defn list-order-by-created-at ...)`
  - --------------------- `sql/<dialect>/<schema>/<table>/list-by-status.sql`
  -   ------------------> `sql/<dialect>/<schema>/<table>/core.clj`  `(defn list-by-status ...)`
